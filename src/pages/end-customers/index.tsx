...
import { getEndCustomers, deleteEndCustomerById } from 'apiSdk/end-customers';
import { EndCustomerInterface } from 'interfaces/end-customer';

type ColumnType = ColumnDef<EndCustomerInterface, unknown>;

interface EndCustomerListPageProps {
...
export function EndCustomerListPage(props: EndCustomerListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<EndCustomerInterface>>(
    () => `/end-customers?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: EndCustomerInterface) => {
    if (hasAccess('end_customer', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/end-customers/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/end-customers/view/${record.id}`} passHref legacyBehavior>
            <Button
          ...
          {hasAccess('end_customer', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/end-customers/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('end_customer', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
  ...
  return (
    <Box p={4} rounded="md" shadow="none">
      <AccessInfo entity="end_customer" />
      <Flex justifyContent="space-between" mb={4}>
        <Text as="h1" fontSize="1.875rem" fontWeight="bold" color="base.content" {...titleProps}>
          End Customers
        </Text>
        {hasAccess('end_customer', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/end-customers/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
